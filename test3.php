...
class Trader {
    ...
    public function test(){
        if(count($this->close)<5800) return;
        if($this->nextaction==self::SELL && 
            array_sum(array_slice($this->close,-3800))/3800 > array_sum(array_slice($this->close,-5800))/5800){
            return;
        }
...
//            )
        ...
        if($this->nextaction==self::SELL
            ...
            && end($this->wtlb_red) >= $this->border 
            && end($this->wtlb_red) > end($this->wtlb_green)
//            && array_sum(array_slice($this->close,-180))/180 > array_sum(array_slice($this->close,-360))/630
//            && (
